...
`config.alloy` force-sync exists for the same reason and was added late: under `copy_if_missing` the log pipeline was **frozen at install time forever** — the CRS severity mapping added in `60ea290` reached *no* existing deployment despite docs describing it as live.

`loki/local-config.yaml` was the **last** config to make that move (Aug 2026) and shows the cost of waiting: while it was `copy_if_missing`, disabling Loki's built-in level detection had to travel as a **CLI flag** in `docker-compose.monitoring.yml` (`-validation.discover-log-levels=false`) because a config key would have reached nobody — that flag is a workaround, not a design. The only blocker was per-stream retention, documented as operator-tunable; it now lives in `.env` (`LOKI_RETENTION_DEFAULT`/`_APPSERVER`/`_DBSERVER`/`_WEBSERVER`) and the file renders from `.tpl` exactly like `netdata.conf`. **The migration reads the old values out of the operator's existing `local-config.yaml` (awk) instead of writing repo constants** — a constant would silently reset someone's tuned retention on a plain `git pull && make up`, which is precisely what the backwards-compat contract forbids. Two guards, both because Loki with a malformed `duration` **doesn't start at all** (an operator would read that as broken monitoring, not a bad `.env`): only `<number><unit>` passes through, and the render refuses to swap the file if any `__RETENTION_*` placeholder survived. Fresh installs: `init-configs` runs `ensure-config-files` *before* `.env` exists, so the first render uses repo defaults and the first `make up` adds the vars and re-renders byte-identical (`cmp` sees no change) — same sequence as `ALTCHA_HMAC_KEY`, so `init-configs` gets no second copy.
...
Requests that are not attacks but are **not ours** (CRS passes them, correctly). All in versioned bind-mounts → `git pull && make up`, no `.env` migration. Four rules + `server_tokens`, all covered by `make test-waf` (37 cases). Detail: `docs/architektura/utwardzenie-brzegu.md`.
...
**Outbound rules (`RESPONSE-95x`) are the trap.** They scan the *response body*, `ANOMALY_OUTBOUND=4`, and a single `severity:ERROR` hit contributes exactly 4 → one match blocks. Worse, `959100` fires in `phase:4`, when nginx has usually **already sent headers**: it cannot return 403 (nor our `error_page 403 → 444`), so the response is truncated and the access log records **`500 0`** — a symptom that points nowhere near the WAF. Rule `10004` therefore strips `950000-959999` for `/grafana/`, `/dozzle/`, `/flower/`, `/netdata/` (third-party dashboards behind `auth_request`, whose HTML is not ours to police); inbound protection on those paths stays fully enforced. Rule `10005` disables `920280` for h2/h3 because the connector cannot see the `:authority` pseudo-header. **`make test-waf` does not cover the `10004` exclusion** — locally `auth_request` suppresses response inspection on the panel locations (on production it does not; cause unexplained), so a `/grafana/` *response-body* case would be green regardless of the rule. That caveat is **outbound-only** and does not extend to `10006` below — inbound rules do fire locally, and that check is verified red-without-the-rule.

**`/grafana/` is exempt from the WAF entirely (rule `10006`, `ctl:ruleEngine=Off`) — this is not the same thing as `10004`.** Symptom when it's missing: **every panel on every dashboard shows "No data"** while Grafana itself looks healthy — only the `POST /grafana/api/ds/query` calls are 403'd, GETs pass. Cause (measured on production 2026-08-05): the request body is a **LogQL query**, and `Log Level = All` expands `$level` into `detected_level=~"critical|debug|error|info|unknown|warn"`. The substring **`|debug`** is a shell metacharacter followed by a DOS command, so CRS `932110` (Windows command injection) matches — `Matched Data: |debug found within ARGS:json.queries.array_0.expr` — severity CRITICAL = 5 at `ANOMALY_INBOUND` 5, so **one** hit blocks. **Don't "fix" this by excluding `932110`**: the coupling is structural — LogQL is metacharacter-dense by nature, and the WAF dashboard's cross-filters put *literal attack payloads* into the query, i.e. they feed the WAF its own findings. Every new dashboard field would add another rule to exclude. Exempting is safe because `location /grafana/` sits behind `auth_request /_bpp_superuser_auth` — only a logged-in BPP **superuser** can reach it. `10004` is deliberately left listing `/grafana/` too (redundant now, but the outbound exclusion must survive if anyone narrows `10006`), and it remains the *only* exclusion for `/dozzle/`, `/flower/`, `/netdata/` — **don't extend `10006` to them without measuring**. Covered by two `make test-waf` checks that must stay paired: the payload blocked *outside* `/grafana/` (else the test passes with CRS off entirely) and passing *on* it, asserting **403 specifically** — the main battery scores any HTTP code as pass, so a WAF block would read as success there. h3 **is** covered, by a separate check (not the `--http1.1` battery, which is blind to that class): QUIC client from inside the docker network, verdict read from the **access log** — `444` on `HTTP/3.0` is a real block, zero `HTTP/3.0` lines means the client lost the handshake (retried; amd64 emulation drops ~1 in 5). Detail: `docs/architektura/waf.md`.

### Grafana dashboards — two traps that both look like "everything is No data"

Both were live in shipped dashboards; both were invisible to assertions that only grepped the JSON.

**1. A data link starting with `?` loses the dashboard path.** `"url": "?var-service=${…}"` *looks* like a valid relative URL (RFC 3986 would keep the path) but Grafana does no URL resolution — it hands the string to `locationService.push()`, whose router parses `?var-x=1` as `pathname: ''` → **Grafana home page**. The Log Monitoring cross-filter was dead this way from `989bf83` through `ef6e8ad`. **Always write `"/d/<uid>?…"`** — verified to work under the production subpath too (`GF_SERVER_SERVE_FROM_SUB_PATH`, Grafana prepends the basename).

**Interpolate with `${var:queryparam}`, not a hand-written `var-x=${var}`.** Only `:queryparam` expands multi-value variables into repeated `var-x=a&var-x=b` and preserves the `$__all` state; `service`/`container`/`level` are multi. Use `:percentencode` on the `${__data.fields.X}` accessor — without it a `?` inside the value (scanner paths are full of them) splits the query string. Every link must carry the **complete** variable set, otherwise a click silently resets the other filters.

**2. Grafana's own "Filter for value" breaks any dashboard querying structured metadata.** The ad-hoc filter is injected into the **stream selector** (`{job="docker", service="webserver", modsec_msg="…"}`), and `modsec_*` are structured metadata by design (`modsec_uri` × `modsec_client` as stream labels would blow up index cardinality) — so no stream matches and **every** panel goes empty at once. Cause, in Grafana 12.4.2: `datasource.ts` → `addAdHocFilters()` calls `addLabelToQuery()` **without** the `labelType` argument, and `modifyQuery.ts` without it guesses from `parserPositions.length === 0` → stream selector. The Logs panel gets it right only because it knows the field type from Loki's response.

**The button cannot be removed from dashboard JSON** — `setDashboardPanelContext.ts` sets `onAddAdHocFilter` unconditionally, and `filterable: false` (already on every table) governs the *column* filter. **The fix is the `else` branch of that same guess**: `if (parserPositions.length === 0) addFilterToStreamSelector(...) else addFilterAsLabelFilter(...)`. Every `waf.json` query therefore ends with a **no-op parser**, `| logfmt bpp_noop="__bpp_noop__" | drop bpp_noop`, so the filter lands in the pipeline and simply works. Measured on a stand (loki 3.7.1 + grafana 12.4.2, 60 audit entries): without it `60 → "No data"` and 11 empty panels, with it `60 → 20`.

Three ways to get that parser wrong, all measured against the real lines in `tests/fixtures/alloy-loglines.txt`: `| pattern "<_>"` is **rejected by Loki** (`at least one capture is required`); `| json` stamps `__error__` on the error.log twin (not JSON); bare `| logfmt` parses clean but **extracts junk** — `level`, `msg`, `ts`, `duration`, `___export`, `___plik` — and `level` would compete with `detected_level`, whose single source must stay `config.alloy`. An explicit label from a key that never occurs extracts nothing; `drop` clears the empty label. **The parser must stay the LAST pipeline stage** — there it only chews lines that already passed `modsec_src` and the variable filters, instead of the whole webserver stream; a separate assertion guards the position independently of the presence check. `error-monitoring.json` deliberately does **not** get this: its ad-hoc filters target `service`/`container`/`detected_level`, which are real stream labels that already work, and moving them into the pipeline would trade an index lookup for a full scan on the one dashboard that queries *all* containers.

Our own filters stay too: textbox variables + row data links (`waf.json` has five: rule/attack/client/uri/score).

Field-set asymmetry that any new WAF filter must respect: `modsec_attack`, `modsec_rules`, `modsec_code`, `modsec_method`, `modsec_paranoia` exist **only** on the audit twin; the error.log twin has the other 11. Applying `$attack` to the logs panel would empty it whenever a category is selected — i.e. reproduce the very bug. Guarded by `test_waf_crossfilter` in `tests/test_makefile.sh`. Detail: `docs/architektura/waf.md#pulapka-filtrow-ad-hoc`.
...
**Image change detection compares entries pairwise, and `none` transitions never count.** A missing local *tag* is not a newer version. Production 2026-08-05: `mcuadros/ofelia:0.3.21` lost its tag after every deploy — `docker system prune -af` (end of `make up`) can't delete an image a running container holds, so it drops the reference instead — and the next cycle read `none → ID` as an update and redeployed **all of production, every cycle, forever**. The image never left the disk (the pull took 2 s: only the tag came back). Skipping those transitions loses nothing: a genuinely new image always yields `ID_old → ID_new` because a running stack has its tags in place, and a *new compose service* arrives with a commit, so `git_changed` fires anyway. Also **log which image changed** — the old nameless "Wykryto nowszy obraz Docker." over 18 images was undiagnosable. Note `docker image inspect` on a missing tag prints an **empty line on stdout** before failing, so `$(… || echo none)` used to yield a two-line entry; use `| head -1` + `${id:-none}`.

**Loop self-restart — the only frozen thing is the loop itself.** `make autoupdate` expanded its `while` body and `$(AUTOUPDATE_INTERVAL)` at start, so changes to `Makefile`/`mk/deployment.mk` don't reach a running session. The script fingerprints those two files (`git rev-parse HEAD:<file>`) **before** the pull and, if they changed, ends its own screen session so the cron watchdog respawns it. Four hard conditions: fingerprint differs, `AUTOUPDATE_SELF_RESTART != 0`, running under `screen` (`$STY`, which also yields the session name — make doesn't export `AUTOUPDATE_SCREEN_NAME`), and a watchdog marker `# BPP-AUTOUPDATE` in `crontab -l`. Missing screen/watchdog → **warn, don't kill**: a silently dead loop is worse than a stale one. **CRITICAL: release the lock dir (and clear the `trap`) *before* `exec screen -X quit`** — that kill gives no chance to run `trap EXIT`, and an orphaned lock stops every later cycle with "inny cykl trwa", i.e. auto-update dies with one log line as the only trace. Guarded by an explicit assertion in `scripts/test-autoupdate.sh`. Don't try to make the loop `break` on an exit code instead: that edit lives in the very file a running session has already frozen, so it could never deploy itself.
