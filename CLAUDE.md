...
**The multiseek report title — `ARGS:suggested-title` on `/bpp/build_search/` and `ARGS:value` on `/bpp/update-multiseek-title/` — is excluded by tag from LFI/RFI/RCE/PHP/SQLi (rule `10011`); XSS deliberately stays.** Symptom: "Szukaj publikacji" / "Pokaż wszystkie publikacje" on one particular unit/author/source page dies on an instant torn connection (`POST /bpp/build_search/ … 444 0`) — the field carries the object's `__str__`, e.g. `… [09.2024...] (WBIOTECH)` lights `933210`. **Anti-fixes — do NOT:** (1) drop the path to `DetectionOnly` like `10009` — both endpoints are **anonymous** and `@csrf_exempt`, the other args must stay protected; (2) remove `attack-xss` "for consistency" — the title is rendered `|safe` and CSRF can plant it into a victim's session, so the WAF is the second layer behind `nh3`; (3) swap tags for a rule-ID list — it's free text, the same losing game as `10009`. Detail: `docs/architektura/waf.md#tytul-raportu`.
...
**Orphaned lock takeover — only on proof the owner is dead.** SIGKILL/OOM and host reboot skip `trap EXIT` (SIGHUP/SIGTERM don't), so the lock dir carries `owner` (`pid`, `boot`, `started`) and a busy lock is taken over only when the boot id differs, the PID is gone, or the PID is no longer `autoupdate.sh`. **Anti-fixes — do NOT:** (1) take over a *live* owner by age — past `AUTOUPDATE_LOCK_MAX_AGE_MINUTES` (120) we only log a warning, since a takeover means two parallel `make run`s; age alone decides only for an `owner`-less lock from an older script; (2) `rm -rf` the lock dir — remove `owner`/`owner.tmp` then `rmdir`; (3) drop the `.takeover` mutex or the `ps -p $$` self-probe (without it a missing/busybox `ps` makes every live lock look dead); (4) release with a bare `rmdir` — it fails on the non-empty dir, and the next cycle's takeover silently hides that. Each guard has a mutation-verified case in `scripts/test-autoupdate.sh`. Detail: `docs/rozwoj/pulapki-wdrozenia.md`.

**The loop has no SSH agent.** Symptom: every cycle logs `OSTRZEZENIE: 'git fetch' nieudany` — images still deploy, commits never arrive. Fix is `make git-bez-klucza` (origin fetch over HTTPS, `scripts/git-remote.sh`; public repo only; private → deploy key). **Don't** forward/symlink `SSH_AUTH_SOCK` into the screen session (the cron-respawned session never has one) or suggest a PAT (expires silently). Any git/ssh call in the loop must never be able to prompt — under screen's pseudo-TTY a prompt hangs the cycle *while it holds the lock*; `git-remote.sh` checks with `GIT_TERMINAL_PROMPT=0` + `BatchMode=yes` (`make test-git-remote`).
