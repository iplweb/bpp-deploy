...
**The button cannot be removed from dashboard JSON** — `setDashboardPanelContext.ts` sets `onAddAdHocFilter` unconditionally, and `filterable: false` (already on every table) governs the *column* filter. The only defense is giving the operator working filters of our own: textbox variables + row data links (`waf.json` has five: rule/attack/client/uri/score). **Don't add a parser to queries to "fix" the placement** — `| pattern "<_>"` is rejected by Loki and `| logfmt`/`| json` would stamp `__error__` on half the lines.
